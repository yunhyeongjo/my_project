...
onMounted(() => {
  ...
  const crtLoveTl = () => {
    ...
    return (new mojs.Timeline).add([
      ...
      new mojs.Shape({
        ...
        x: -64,
        y: -82, //
        scale: { 0: 0.90, easing: easingHeart },
        duration: 500
      }).then({
        x: { to: -62, easing },
  ...
  const toggleSound = () => {
    soundOn = !soundOn
    el.blup.volume = soundOn ? volume : 0
    el.blop.volume = soundOn ? volume : 0
    el.sound.classList.toggle('sound--off', !soundOn)
  }
...
.container {
  width: 50rem;
  height: 20rem;
  position: relative; // 또는 relative 도 OK
}
